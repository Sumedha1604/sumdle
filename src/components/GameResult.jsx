function GameResult({ attempts, gameStatus, onPlayAgain, solution }) {
  ...
  return (
    <div className="result-overlay">
      <section className={`result-card result-card--${gameStatus}`} role="dialog" aria-modal="true" aria-labelledby="result-title">
        ...
        <p className="result-kicker">{won ? 'a little win' : 'tomorrow is another try'}</p>
        ...
        <button className="play-again-button" type="button" onClick={onPlayAgain} autoFocus>play again</button>
      </section>
    </div>
  )
}
