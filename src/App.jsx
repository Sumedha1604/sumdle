...
function App() {
  ...
  return <main className="game-shell">
    ...
    <section className="game-card" aria-label="Sumdle game board"><div className="card-decoration decoration-top" aria-hidden="true">{decorations.card[0]}</div><div className="card-decoration decoration-bottom" aria-hidden="true">{decorations.card[1]}</div>
      ...
      {gameStatus === GAME_STATUS.playing && <div className="hint-area"><div className="hint-control-row"><HintFlower decoration={decorations.hint} state={mascotState} /><button className={`hint-button${hintCount < 2 && !hint ? ' hint-button--available' : ''}`} type="button" onClick={requestHint} disabled={isLoading || isSubmitting || hintCount >= 2}>{hintCount >= 2 ? 'all tiny hints used' : decorations.hintLabel ?? `hint ${hintCount ? '(one more)' : ''}`}</button></div><div className={`hint-reveal${hint ? ' hint-reveal--open' : ''}`}><div className="hint-reveal-inner"><div className="hint-toast-row"><p aria-hidden={!hint} className="hint-toast" role="status"><strong>tiny hint ✦</strong>{hint}</p></div></div></div></div>}
